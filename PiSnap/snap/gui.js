...
IDE_Morph.prototype.projectMenu = function () {
    ...
    menu.addLine();
    menu.addItem(
        'Export project on file...',
        function () {
            if (myself.projectName) {
                myself.exportProjectOnFile(myself.projectName, null);
            } else {
                myself.prompt('Export Project As...', function (name) {
                    myself.exportProjectOnFile(name);
                }, null, 'exportProject');
            }
        },
        'show project data as XML\nin in browser\'s download direction'
        //shiftClicked ? new Color(100, 0, 0) : null
    );
    ...
    menu.addItem(
        ...
        function () {
            // read a list of libraries from an external file,
            var libMenu = new MenuMorph(this, 'Import library'),
                libUrl = 'http://'+myhosturl+'/libraries/' +
                    'LIBRARIES';

            function loadLib(name) {
                var url = 'http://'+myhosturl+'/libraries/'
                        + name
                        + '.xml';
                myself.droppedText(myself.getURL(url), name);
            }
...
IDE_Morph.prototype.exportProjectOnFile = function (name, plain) {
    var menu, str;
    if (name) {
        this.setProjectName(name);
        if (Process.prototype.isCatchingErrors) {
            try {
                menu = this.showMessage('Exporting');
                var blob = new Blob([this.serializer.serialize(this.stage)], {type: "text/plain;"});
