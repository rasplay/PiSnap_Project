...
def s2a_fm():
    ...
    # turn on logging
    logging.basicConfig(filename=path+'/log/s2a_fm_debugging.log', format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', filemode='a', level=logging.DEBUG)
    ...
    logging.info("Please wait for Total Arduino Pin Discovery to complete. This can take up to 30 additional seconds.")
    ...
    while not pin_capability:
        if time.time() - start_time > 30:
            print ''
            logging.debug("Could not determine pin capability - exiting.")
            firmata.close()
    ...
    logging.info("Arduino Total Pin Discovery completed in %d seconds" % (int(time.time() - start_time)))
